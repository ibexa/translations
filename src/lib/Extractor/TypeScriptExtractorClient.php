...
use Ibexa\Translations\Exception\TypeScriptExtractorException;
use JsonException;
...
final class TypeScriptExtractorClient
{
    ...
    private function assertRuntimeIsReady(): void
    {
        ...
        if (!$runtimeCheckProcess->isSuccessful()) {
            throw new TypeScriptExtractorException(sprintf(
                'TypeScript translation extractor runtime is not available. Please ensure `%s` is installed and `@typescript-eslint/typescript-estree` is available for the translations package. %s',
                $this->nodeBinary,
    ...
    private function decodeResponse(string $responseLine): array
    {
        ...
        if (array_key_exists('messages', $responseData)) {
            $response['messages'] = $responseData['messages'];
        }

        if (array_key_exists('warnings', $responseData)) {
            $response['warnings'] = $responseData['warnings'];
        }

        if (array_key_exists('error', $responseData)) {
            if (!is_string($responseData['error'])) {
                return ['error' => 'Extractor error must be a string.'];
            }
