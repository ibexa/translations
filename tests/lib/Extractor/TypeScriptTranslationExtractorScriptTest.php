...
final class TypeScriptTranslationExtractorScriptTest extends TestCase
{
    ...
    public function testExtractsExpectedMessages(
        ...
        self::assertIsArray($extractionResponse);

        $messages = $extractionResponse['messages'] ?? null;
        self::assertIsArray($messages);
        self::assertSame($expectedMessages, $messages);

        $warnings = $extractionResponse['warnings'] ?? null;
        self::assertIsArray($warnings);
        self::assertCount(count($expectedWarningSubstrings), $warnings);

        foreach ($expectedWarningSubstrings as $index => $substring) {
            $warning = $warnings[$index] ?? null;
            self::assertIsString($warning);
            self::assertStringContainsString($substring, $warning);
        }
    ...
    public function testServeModeProcessesMultipleFilesSequentiallyAndSurvivesPerFileErrors(): void
    {
        ...
        $firstResponse = json_decode($lines[0], true);
        self::assertIsArray($firstResponse);
        self::assertSame($expectedMessage, $firstResponse['messages'] ?? null);

        $secondResponse = json_decode($lines[1], true);
        self::assertIsArray($secondResponse);
        self::assertArrayHasKey('error', $secondResponse);

        $thirdResponse = json_decode($lines[2], true);
        self::assertIsArray($thirdResponse);
        self::assertSame($expectedMessage, $thirdResponse['messages'] ?? null);
    ...
    {
        return dirname(__DIR__, 3) . '/src/lib/Extractor/typescript_translation_extractor.mjs';
    }
}
